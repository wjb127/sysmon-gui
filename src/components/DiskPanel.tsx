...
export function DiskPanel({ disks }: Props) {
  ...
  // 크기 계산 중인 경로 집합
  const [pendingSizes, setPendingSizes] = useState<Set<string>>(new Set());

  async function navigateTo(path: string, newCrumbs: { name: string; path: string }[]) {
    setLoading(true);
    setError(null);
    setPendingSizes(new Set());
    try {
  ...
  // 폴더 하나의 크기를 온디맨드로 계산
  async function calcSize(entry: DirEntry) {
    if (pendingSizes.has(entry.path)) return;
    setPendingSizes((prev) => new Set(prev).add(entry.path));
    try {
      const size = await invoke<number>("calc_dir_size", { path: entry.path });
      setEntries((prev) =>
        prev.map((e) => (e.path === entry.path ? { ...e, size_bytes: size } : e))
      );
    } finally {
  ...
  function enterVolume(mount: string) {
    navigateTo(mount, [{ name: mount, path: mount }]);
  }

  function enterDir(entry: DirEntry) {
    navigateTo(entry.path, [...breadcrumbs, { name: entry.name, path: entry.path }]);
  }

  function navigateToCrumb(index: number) {
    const crumb = breadcrumbs[index];
    navigateTo(crumb.path, breadcrumbs.slice(0, index + 1));
  }
  ...
  function goBack() {
    if (breadcrumbs.length <= 1) {
      ...
      navigateTo(prev.path, newCrumbs);
    }
  }
            ...
            {disks.map((disk, i) => (
              ...
              >
                ...
                <div className="w-full h-1.5 bg-[#2a2d3e] rounded-full overflow-hidden mb-1.5">
                  <div className={`h-full rounded-full transition-all duration-300 ${barColor(disk.percent)}`}
  ...
  return (
    <div className="flex flex-col" style={{ maxHeight: 280 }}>
        ...
        <div className="flex items-center gap-1 min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1 text-xs min-w-0 overflow-hidden">
            {breadcrumbs.length > 2 && (
              <>
                <button onClick={() => navigateToCrumb(0)} className="text-gray-500 hover:text-gray-300 flex-shrink-0">
                  {breadcrumbs[0].name}
                </button>
              ...
              return (
                <span key={realIdx} className="flex items-center gap-1 min-w-0">
                  ...
                  {isLast
                    ...
                    : <button onClick={() => navigateToCrumb(realIdx)} className="text-gray-400 hover:text-gray-200 truncate">{crumb.name}</button>
                  }
                </span>
              );
            })}
      ...
      <div className="overflow-y-auto flex-1">
        {loading && <div className="flex items-center justify-center py-6"><span className="text-xs text-gray-500">Loading...</span></div>}
        ...
        {!loading && !error && visible.map((entry, i) => {
          const isPending = pendingSizes.has(entry.path);
          return (
            <div
              key={i}
              onClick={() => entry.is_dir && enterDir(entry)}
              className={`flex items-center gap-2 px-3 py-1.5 border-b border-[#1e2235] last:border-0 ${
                entry.is_dir ? "cursor-pointer hover:bg-[#1e2235] group" : "cursor-default"
              } ${entry.is_hidden ? "opacity-50" : ""}`}
            >
              <span className="flex-shrink-0 text-sm">{entry.is_dir ? "📁" : "📄"}</span>
              ...
              <div className="flex-shrink-0 text-right" onClick={(e) => e.stopPropagation()}>
                {entry.is_dir ? (
                  entry.size_bytes > 0 ? (
                    // 계산 완료
                    <div>
                      <div className="text-[10px] text-gray-400 tabular-nums">{formatBytes(entry.size_bytes)}</div>
                      {entry.child_count != null && <div className="text-[9px] text-gray-600">{entry.child_count} items</div>}
                    </div>
                  ) : isPending ? (
                    // 계산 중
                    <span className="text-[10px] text-gray-500 animate-pulse">계산 중…</span>
                  ) : (
                    // 계산 버튼
                    <button
                      onClick={() => calcSize(entry)}
                      className="text-[10px] text-gray-600 hover:text-indigo-400 px-1.5 py-0.5 rounded hover:bg-indigo-500/10 transition-colors"
                      title="크기 계산"
                    >
                  ...
                  <span className="text-[10px] text-gray-400 tabular-nums">{formatBytes(entry.size_bytes)}</span>
                )}
              </div>
